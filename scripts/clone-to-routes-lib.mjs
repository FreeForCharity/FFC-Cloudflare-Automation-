...
export function routeSource({
  ...
  const lines = [
    ...
    // `absolute` suppresses the root layout's `title.template`. It is set when
    // the captured title still carries the SITE's own brand, because the
    // template appends `siteConfig.name` — which on a repo that has not been
    // rebranded yet is the FFC template's name, not the charity's. Without it
    // every page renders `Volunteers | Charity | Free For Charity`: the
    // sponsor's name on the charity's own pages, and a title Google truncates.
    // With it the page reproduces the title the source site served.
  ...
  lines.push(
    `  alternates: { canonical: ${tsString(canonical)} },`,
    '}',
    '',
...
function selfTest() {
  ...
  eq(
    ...
    /\n\s*title: '/.test(routeSource({ slug: 'x', title: 'X | Charity', absoluteTitle: true })),
    false,
  );
  eq(
